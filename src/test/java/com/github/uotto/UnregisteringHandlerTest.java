...
public class UnregisteringHandlerTest {
    ...
    public void setUp() throws Exception {
        bus = new Bus(BUS_IDENTIFIER, new SortedHandlerFinder());
    }
    ...
    public void unregisterInHandler() {
        ...
        assertEquals("One correct event should be delivered.", Collections.singletonList(EVENT), catcher.getEvents());

        bus.post(EVENT);
        bus.post(EVENT);
        assertEquals("Shouldn't catch any more events when unregistered.", Collections.singletonList(EVENT), catcher.getEvents());
    }
    ...
    public void unregisterInHandlerWhenEventProduced() throws Exception {
        ...
        bus.register(catcher);
        assertEquals(Collections.singletonList(StringProducer.VALUE), catcher.getEvents());

        bus.post(EVENT);
        bus.post(EVENT);
        assertEquals("Shouldn't catch any more events when unregistered.",
                Collections.singletonList(StringProducer.VALUE), catcher.getEvents());
    }
    ...
    static class SortedHandlerFinder implements HandlerFinder {
        ...
        static Comparator<EventHandler> handlerComparator = (eventHandler, eventHandler1) -> eventHandler.toString().compareTo(eventHandler1.toString());
        ...
        public Map<Class<?>, Set<EventHandler>> findAllSubscribers(Object listener) {
            ...
            Map<Class<?>, Set<EventHandler>> sorted = new HashMap<>();
            for (Map.Entry<Class<?>, Set<EventHandler>> entry : found.entrySet()) {
                SortedSet<EventHandler> handlers = new TreeSet<>(handlerComparator);
